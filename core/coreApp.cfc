<cfcomponent>
	...
	<cfset this.resetAppKey = "resetapp">
	...
	<cffunction name="onRequestStart" access="public" hint="This contains the actual front controller logic">
		...
		<cfargument name="requestState" type="struct" required="false" />
 		<cfscript>
 			 var appEventHandler = 0;
			...
			if(structKeyExists(arguments,"requestState")) {
				reqState = arguments.requestState;
			} else {
				// create a structure to hold current request state
				StructAppend(reqState, form);
			...
			if(not structKeyExists(reqState,"event") or reqState.event eq "")
				reqState.event = this.defaultEvent;
			...
			resetApp = structKeyExists(reqState, this.resetAppKey)
	...
	<cffunction name="startApplication" access="private" returntype="void" hint="This method handles the application startup tasks">
		...
		<cfset var xmlDoc = 0>
		<cfset var lockName = "coreAppStart_#hash(getBaseTemplatePath())#">
