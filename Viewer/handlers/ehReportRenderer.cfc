<cfcomponent name="ehReportRenderer" extends="ehBase">
	...
	<cffunction name="dspMain" access="public" returntype="void">
		<cfscript>
			...
			var maxRows = getValue("maxRows",999999);
			...
			try { 
				...
				// if function params are passed to viewer, then overwrite them
				if(structKeyExists(session,"functionParameters") and session.FunctionParameters neq "")
					oReport.setFunctionParameters(FunctionParameters);

				if(reportMode eq "Display")
					oReportRenderBean.setJSCallBack("window.parent.setupReport(document)");
				...
				if(findNoCase("Access denied",e.detail) or findNoCase("Login failed",e.detail)) 
					setNextEvent("ehReportRenderer.dspError","accessDenied=true");
				else
	...
	<cffunction name="dspError" access="public" returntype="void">
		...
		<cfset setValue("accessDenied",getValue("accessDenied"))>
